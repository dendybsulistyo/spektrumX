<x-app-layout>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 mb-4">
        <form method="GET" class="flex flex-wrap items-end gap-3">
            ...
            @if ($from || $to)
                <a href="{{ route('dashboard') }}" class="px-4 py-2 text-sm text-gray-500 hover:underline">Reset</a>
            @endif
            <p class="text-xs text-gray-400 ml-auto">Kartu ringkasan &amp; tabel di bawah mengikuti filter tanggal (berdasarkan tanggal masuk order).</p>
        </form>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div class="p-4 border-b border-gray-200">
            ...
            <p class="text-xs text-gray-400 mt-0.5">
                @if ($from || $to)
                    Order dari {{ $from ?: 'awal' }} sampai {{ $to ?: 'sekarang' }}, file -> kasir → layout → cetak → finishing -> QC → bungkus -> pengambilan.
                @else
                    30 order terbaru, file -> kasir → layout → cetak → finishing -> QC → bungkus -> pengambilan.
                @endif
            </p>
        </div>
