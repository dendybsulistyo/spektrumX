                ...
                <table class="w-full text-[13px] min-w-[560px]">
                    ...
                    <tbody class="divide-y">
                        ...
                        @forelse ($outdoorOrders as $order)
                            @foreach ($order->items as $item)
                                ...
                                <tr>
                                    ...
                                    <td class="px-2 py-1.5 text-gray-500 whitespace-nowrap" style="min-width: 110px;">{{ $item->NmFile ?: '-' }}</td>
                                        ...
                                        @can('order-desain.manage')
                                            <form method="POST" action="{{ route('order-desain.gabungan', $item) }}">
                                                ...
                                                <input type="text" name="gabungan" value="{{ $item->gabungan }}" maxlength="255"
                                                       ...
                                                       class="w-28 rounded-md border-gray-300 text-xs py-1 px-1.5">
                                            </form>
                                        @else
                        ...
                        @foreach ($outdoorNeedsReply as $order)
                            ...
                            @endforeach
                        @endforeach
                        {{-- $outdoorNeedsReply doesn't get an x-order-rework button — those orders already advanced past desain, and the rework action belongs to whichever stage they're currently sitting in. --}}
