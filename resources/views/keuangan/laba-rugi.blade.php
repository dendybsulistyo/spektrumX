<x-app-layout>
    ...
    @push('styles')
        ...
        <style>
            ...
            #industry-laba-rugi .lr-group-title { font-family: var(--font-heading); font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.04em; color: var(--color-text-muted, #6b7280); padding: var(--space-3) 0 var(--space-2); }
        ...
        <div style="max-width: 1100px; margin: 0 auto; display: flex; flex-direction: column; gap: var(--space-6);">
            ...
            <div class="blueprint" style="padding: var(--space-4);">
                ...
                <form method="GET" style="display: flex; align-items: flex-end; gap: var(--space-3); flex-wrap: wrap;">
                    ...
                    <p class="text-muted" style="font-size: 12px; margin-left: auto;">Periode {{ \Carbon\Carbon::parse($dari)->translatedFormat('d M Y') }} — {{ \Carbon\Carbon::parse($sampai)->translatedFormat('d M Y') }}</p>
                </form>
            </div>
            ...
            <section style="display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--space-4);">
                <div class="card blueprint"><i class="corner tl"></i><i class="corner tr"></i><i class="corner bl"></i><i class="corner br"></i>
                    ...
                    <div class="{{ $labaKotor < 0 ? 'lr-negative' : '' }}" style="font-family: var(--font-heading); font-weight: 600; font-size: 24px; line-height: 1;">{{ $fmt($labaKotor) }}</div>
                    <div class="card-meta">Pendapatan − HPP</div>
                </div>
                <div class="card blueprint"><i class="corner tl"></i><i class="corner tr"></i><i class="corner bl"></i><i class="corner br"></i>
                    <div class="card-kicker">Laba Usaha</div>
                    <div class="{{ $labaUsaha < 0 ? 'lr-negative' : '' }}" style="font-family: var(--font-heading); font-weight: 600; font-size: 24px; line-height: 1;">{{ $fmt($labaUsaha) }}</div>
                ...
                <div class="card blueprint" style="background: var(--color-accent-900); color: var(--color-bg); border-color: var(--color-accent-900);">
                    ...
                    <div style="font-family: var(--font-heading); font-weight: 600; font-size: 24px; line-height: 1;">{{ $fmt($labaBersih) }}</div>
                </div>
            </section>
                ...
                <div style="overflow-x: auto;">
                    <table class="table" style="min-width: 640px;">
                        ...
                        <tbody>
                            ...
                            <tr class="lr-subtotal"><td style="font-size: 15px;">Laba Bersih</td><td style="text-align: right; font-size: 15px;" class="{{ $labaBersih < 0 ? 'lr-negative' : '' }}">{{ $fmt($labaBersih) }}</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
