                ...
                <div x-show="tab === '{{ $tabKey }}'" @if($tabKey!=='indoor') x-cloak @endif class="overflow-x-auto">
                    <table class="w-full text-[13px] min-w-[560px]">
                        ...
                        <tbody class="divide-y">
                                    ...
                                    <td class="px-3 py-2 text-right">
                                        ...
                                        @unless ($pendingRework->has($tabKey.'-'.$order->id))
                                            <form method="POST" action="{{ route('order-qc.update', [$tabKey, $order->id]) }}"
                                                  onsubmit="return confirm('Yakin ingin mengirim order {{ $order->NoOrder }} ke Bungkus?')" class="inline">
                                                @csrf
                            ...
                            @empty
                                <tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">Tidak ada order di antrian QC.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
