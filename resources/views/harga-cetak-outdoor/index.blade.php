<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800">Harga Cetak Outdoor</h2>
            <a href="{{ route('harga-cetak-outdoor.create') }}"
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-4 border-b">
            <form method="GET" class="flex gap-2">
                ...
                @if (request('search'))
                    <a href="{{ route('harga-cetak-outdoor.index') }}" class="px-4 py-2 text-sm text-gray-500 hover:underline">Reset</a>
                @endif
            </form>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-left text-xs uppercase text-gray-500">
                <tr>
                    ...
                    <th class="px-4 py-3">Kode Cetak</th>
            ...
            <tbody class="divide-y">
                @forelse ($hargaCetakOutdoor as $item)
                    <tr>
                        <td class="px-4 py-3 text-gray-400">{{ $hargaCetakOutdoor->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3">{{ $item->KdCtk }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('harga-cetak-outdoor.edit', $item) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125" /></svg></a>
                            <form method="POST" action="{{ route('harga-cetak-outdoor.destroy', $item) }}" class="inline"
                                  onsubmit="return confirm('Hapus harga cetak {{ $item->KdCtk }}?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-400">Belum ada data harga cetak.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
