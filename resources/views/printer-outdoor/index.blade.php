<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h2 class="font-semibold text-xl text-gray-800">Data Printer Outdoor</h2>
            <a href="{{ route('printer-outdoor.create') }}"
               class="inline-flex items-center px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-md hover:bg-gray-700">
                + Tambah Printer Outdoor
            </a>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-4 border-b">
            <form method="GET" class="flex gap-2">
                ...
                @if (request('search'))
                    <a href="{{ route('printer-outdoor.index') }}" class="px-4 py-2 text-sm text-gray-500 hover:underline">Reset</a>
                @endif
            </form>
        </div>
