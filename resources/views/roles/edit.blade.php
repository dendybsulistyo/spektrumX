<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Edit Role: {{ $role->label }}</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl">
        <form method="POST" action="{{ route('roles.update', $role) }}" class="space-y-4">
            ...
            <div class="pt-2 flex gap-3">
                ...
                <a href="{{ route('roles.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:underline">Batal</a>
            </div>
        </form>
    </div>
</x-app-layout>
