<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Pengaturan Jasa Potong</h2>
    </x-slot>

    <div class="bg-white rounded-lg shadow-sm p-6 max-w-xl">
        <div class="mb-6 p-4 bg-gray-50 rounded-md border border-gray-200">
            <p class="text-sm text-gray-700 font-medium mb-1">Rumus perhitungan Ongkos Jasa Potong</p>
            <p class="text-sm text-gray-600 font-mono">Ongkos = ((Pisau Turun × Jumlah Kertas × Tebal Kertas) / 10) + X</p>
            <p class="text-xs text-gray-400 mt-2">
                Berlaku untuk produk dengan kode "Cara Hitung Harga Order" = 4 (Qty = Harga).
                Pisau Turun, Jumlah Kertas, dan Tebal Kertas diisi per item saat membuat order.
                X adalah biaya tetap di bawah ini, berlaku untuk semua produk berkode tersebut.
            </p>
        </div>

        <form method="POST" action="{{ route('jasa-potong.update') }}" class="space-y-4">
            @csrf
            @method('PUT')
