                ...
                <div x-show="tab === '{{ $tabKey }}'" @if($tabKey!=='indoor') x-cloak @endif class="overflow-x-auto">
                    <table class="w-full text-[13px] min-w-[560px]">
                        ...
                        <tbody class="divide-y">
                                        ...
                                        <div class="flex items-center justify-end gap-1.5">
                                            ...
                                            @unless ($pendingRework->has($tabKey.'-'.$order->id))
                                                <form method="POST" action="{{ route('order-bungkus.update', [$tabKey, $order->id]) }}"
                                                      onsubmit="return confirm('Yakin ingin mengirim order {{ $order->NoOrder }} ke Pengambilan?')" class="inline">
                                                    @csrf
                            ...
                            @empty
                                <tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">Tidak ada order di antrian bungkus.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
