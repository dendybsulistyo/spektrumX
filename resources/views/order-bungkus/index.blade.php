<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Antrian Bungkus</h2>
    </x-slot>
    ...
    @push('styles')
        ...
        <style>
            ...
            #industry-bungkus .seg-tab { display: inline-flex; align-items: center; gap: 6px; padding: 7px 14px; font-family: var(--font-heading); font-weight: 600; font-size: 13px; letter-spacing: 0.02em; cursor: pointer; border: 1px solid var(--color-divider); border-right: none; background: transparent; color: var(--color-text); }
            ...
            #industry-bungkus .in-btn { display: inline-flex; align-items: center; gap: 4px; font-family: var(--font-heading); font-weight: 600; font-size: 12px; padding: 5px 10px; background: var(--color-accent); color: var(--color-bg); border: 1px solid var(--color-accent); cursor: pointer; white-space: nowrap; }
                    ...
                    <div style="overflow-x: auto;">
                        <table class="table" style="min-width: 560px;">
                            ...
                            <tbody>
                                        ...
                                        <td style="text-align: right;">
                                            <div style="display: inline-flex; align-items: center; gap: 6px;">
                                                ...
                                                <x-order-rework :type="$tabKey" :order-id="$order->id" :no-order="$order->NoOrder"
                                                                 current-stage="bungkus"
                                                                 :pending="$pendingRework->get($tabKey.'-'.$order->id)"
                                                                 :can-approve="$canApproveRework" />
                                                @unless ($pendingRework->has($tabKey.'-'.$order->id))
                                                    <form method="POST" action="{{ route('order-bungkus.update', [$tabKey, $order->id]) }}"
                                                          onsubmit="return confirm('Yakin ingin mengirim order {{ $order->NoOrder }} ke Pengambilan?')">
                                                        @csrf
                                                        <input type="hidden" name="action" value="selesai">
                                                        <button type="submit" class="in-btn">Kirim Pengambilan</button>
                                                    </form>
                                                @endunless
                                            </div>
                                        </td>
                                ...
                                @empty
                                    <tr><td colspan="6" class="text-muted" style="text-align: center; padding: var(--space-6);">Tidak ada order di antrian bungkus.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
