    ...
    <div>
        ...
        <select id="isPjLb" name="isPjLb" required
            ...
            @foreach (\App\Models\Produk::PJLB_LABELS as $code => $label)
                <option value="{{ $code }}" @selected((int) old('isPjLb', $produk?->isPjLb) === $code)>{{ $code }} — {{ $label }}</option>
            @endforeach
        </select>
        <x-input-error :messages="$errors->get('isPjLb')" class="mt-1" />
    </div>
    <label class="flex items-center gap-2">
        <input type="checkbox" name="isHPilih" value="1" {{ old('isHPilih', $produk?->isHPilih) == 1 ? 'checked' : '' }}
