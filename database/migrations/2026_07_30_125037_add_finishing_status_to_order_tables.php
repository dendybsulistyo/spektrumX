...
{
    private const OLD_ENUM = "ENUM('baru','dibayar','desain','cetak','qc','siap_diambil','selesai','batal')";

    private const NEW_ENUM = "ENUM('baru','dibayar','desain','cetak','finishing','qc','siap_diambil','selesai','batal')";
    ...
    {
        foreach (['order_indoor' => 'selesai', 'order_outdoor' => 'selesai', 'order_artwork' => 'baru'] as $table => $default) {
            DB::statement("UPDATE {$table} SET status = 'cetak' WHERE status = 'finishing'");
