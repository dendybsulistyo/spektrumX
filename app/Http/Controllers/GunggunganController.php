...
class GunggunganController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $rawDay = $request->string('tanggal')->toString();
        $day = ctype_digit($rawDay) ? (int) $rawDay : 0;
        ...
        $calendar = collect(range(0, $start->diffInDays($end)))->map(function (int $offset) use ($start, $year, $month, $daily) {
            ...
            $current = $date->year === $year && $date->month === $month;
            $row = $current ? $daily->get($date->toDateString()) : null;
    ...
    {
        [$year, $month] = $this->period($request);
        $rows = $this->transactions($year, $month)->select('TglOrder', DB::raw('SUM(total) total'))->groupBy('TglOrder')->pluck('total', 'TglOrder');
        $daily = collect(range(1, Carbon::create($year, $month, 1)->daysInMonth))->map(function (int $day) use ($year, $month, $rows) {
            $total = (float) ($rows[sprintf('%04d-%02d-%02d', $year, $month, $day)] ?? 0);
            $dpp = $total ? round($total / 1.11) : 0;

            return (object) ['day' => $day, 'total' => $total, 'dpp' => $dpp, 'ppn' => $total - $dpp];
        });
    ...
    {
        return [(int) $request->input('tahun', now()->year), min(12, max(1, (int) $request->input('bulan', now()->month)))];
    }
    ...
    private function transactions(int $year, int $month): Builder
    {
        ...
        $invoices = app(OrderDocumentService::class)->invoiceQuery()
            ->whereIn('order_type', ['indoor', 'outdoor'])
            ->whereBetween('issued_at', [$start.' 00:00:00', $end.' 23:59:59'])
            ->selectRaw("order_type jenis, order_id id, DATE(issued_at) TglOrder, number NoOrder, JSON_UNQUOTE(JSON_EXTRACT(snapshot, '$.customer_code')) KdCust, total, 'lunas' status_bayar");

        return DB::query()->fromSub($invoices, 'orders')->select('orders.*');
    }
}
