...
class StoreOrderArtworkRequest extends FormRequest
{
    /**
    ...
    /**
     * Produk with isPjLb === HargaArtwork::PJLB_QTY_ALT (4, "Jasa Potong")
     * price entirely from PisauTurun/JumlahKertas/TebalKertas — require all
     * three for those items specifically, mirroring StoreOrderIndoorRequest.
     */
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            $produkMap = HargaArtwork::whereIn('KdProd', $kodeList)->get()->keyBy('KdProd');

            foreach ($items as $index => $item) {
                $produk = $produkMap->get($item['KdProd'] ?? null);

                if (! $produk || ! $produk->isJasaPotong()) {
                    continue;
                }
